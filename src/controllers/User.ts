...
export const getUsers = async (req: CustomRequest, res: Response) => {
  ...
  try {
    const data = await axios.post(
      ...
      {
        data: {
          year: dayjs(user.dateOfBirth).format('YYYY'),
          month: dayjs(user.dateOfBirth).format('MM'),
          date: dayjs(user.dateOfBirth).format('DD'),
          hours: dayjs(user.dateOfBirth).format('HH'),
          minutes: dayjs(user.dateOfBirth).format('mm'),
          seconds: 0,
